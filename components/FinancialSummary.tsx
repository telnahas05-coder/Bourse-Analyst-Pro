...
const FinancialSummary: React.FC<Props> = ({ data }) => {
  const isFund = data.entityType === 'Fund';

  return (
    <div className="grid grid-cols-1 gap-6">
      
      {/* Top Row: Financials & Composition */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bourse-card p-6 rounded-lg border border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            ...
            {isFund ? 'صورت‌های مالی صندوق' : 'گزارش‌های کدال و حسابرسی'}
            </h3>
            <div className="space-y-4">
                <div className="flex justify-between border-b border-slate-700 pb-2">
                    <span className="text-gray-400">آخرین گزارش:</span>
                    <span className="text-white text-sm truncate max-w-[180px]" title={data.financials.reportType}>{data.financials.reportType}</span>
                </div>
                <div className="flex justify-between border-b border-slate-700 pb-2">
                ...
                <div className="grid grid-cols-3 gap-2 mt-4 text-center">
                    <div className="bg-slate-800 p-2 rounded border border-slate-700">
                        <div className="text-[10px] text-gray-400 mb-1">{isFund ? 'NAV ابطال' : 'سود خالص'}</div>
                        <div className="font-bold text-white text-xs break-words">{data.financials.profitability.netProfit}</div>
                    </div>
                    <div className="bg-slate-800 p-2 rounded border border-slate-700">
                        <div className="text-[10px] text-gray-400 mb-1">EPS / NAV</div>
                        <div className="font-bold text-white text-xs">{data.financials.profitability.eps}</div>
                    </div>
                    <div className="bg-slate-800 p-2 rounded border border-slate-700">
                        <div className="text-[10px] text-gray-400 mb-1">{isFund ? 'رشد NAV' : 'رشد سود'}</div>
        ...
        {/* Composition / Compliance Box - Changes based on Type */}
        <div className="bg-bourse-card p-6 rounded-lg border border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-bourse-accent flex items-center gap-2">
                    ...
                    {isFund ? 'ترکیب دارایی و امیدنامه' : 'ترکیب فروش و محصولات'}
                </h3>
                {isFund && (
            ...
            <div className="h-40 w-full flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <Pie
                        ...
                        cx="50%"
                        cy="50%"
                        innerRadius={isFund ? 35 : 0} // Donut for funds, Pie for companies
                        outerRadius={65}
                        paddingAngle={2}
                        dataKey="value"
                        stroke="none"
            ...
            <div className="mt-2 text-center text-[11px] text-gray-500 flex flex-wrap justify-center gap-2">
                {data.compliance.assetAllocation.slice(0, 4).map((item, i) => (
            ...
            {isFund ? (
                ...
                <div className="mt-4 bg-slate-800 p-3 rounded border border-slate-700 text-center">
                    <span className="text-gray-400 text-xs">وضعیت گزارش فعالیت ماهانه:</span>
                    <p className="text-xs text-gray-200 mt-1">{data.financials.reportType.includes('ماهانه') ? 'بررسی شده ✅' : 'آخرین گزارش ماهانه چک شود ⚠️'}</p>
                </div>
            )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-5 rounded-lg border border-slate-700">
            <h4 className="text-bourse-green font-bold text-sm mb-3 flex items-center gap-2">
                ...
                {isFund ? 'عوامل رشد NAV' : 'محصولات/خدمات سودساز (Profit Drivers)'}
            </h4>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-5 rounded-lg border border-slate-700">
            <h4 className="text-bourse-red font-bold text-sm mb-3 flex items-center gap-2">
                ...
                {isFund ? 'ریسک‌های پرتفوی' : 'زیان‌ده یا ریسکی (Loss Drivers)'}
            </h4>
            <ul className="space-y-2">
